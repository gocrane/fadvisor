...
func (pc *TencentCloud) refreshPricingCache() error {
	now := time.Now()
	defer func() {
		klog.Infof("refreshPricingCache consumed: %v", time.Since(now))
	}()

	items, err := pc.cvm.GetAllZoneInstanceConfigInfos()
	if err != nil {
...
func (pc *TencentCloud) WarmUp() error {
